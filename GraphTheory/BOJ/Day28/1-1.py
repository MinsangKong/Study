...
#백준 17472번 다리만들기2(그래프이론,MST,크루스컬,BFS)
#import sys
...
print(answer)
'''
괜히 삼성 기출문제 A가 아니었다. 섬을 라벨링하고 라벨링한 섬끼리 크루스컬 알고리즘을
적용해서 정렬하면 답이 나올꺼 같았는데 섬을 어떻게 처리해야할지 감도 못잡아서 결국
구글링으로 답을 이해했다. 섬을 라벨링할 때에는 BFS를 적용해서 처리하고 처리한 섬은
크루스컬을 적용해서 해결하는 방식으로 문제를 해결했다.
'''